import java.io.Serializable;
import com.systemwerx.utils.*;
import com.systemwerx.PassGen.*;
import java.util.*;

/* Open KeyStore, read key and compute PassTicket */

public class PassTicketDemo2
{ 
  public static void main(String args[]) 
   {
    PassTicketPureBean pw=new PassTicketPureBean();
    String Result;
    try 
    {

	      String TransTab =  "";
	      keyStoreBean tkb = new keyStoreBean();
	      keyStoreEntry tke;
	      
	      tkb.open(tkb.Key_Supplied,"keystore","1234");
	      
	      tke = tkb.getKey("TSOSYS1");
	      
	      if ( tke == null ) // Empty keystore - just set new key
	      {
		      System.out.println("Cannot find application");
		      return ;
	      }

        pw.setTranslateArray(TransTab);
        pw.setUserID(tke.getUserID());
        pw.setGMTOffset(tke.getGmtTOffset());
        pw.setSessionKey(tke.getSessionKey());
        pw.setApplication("TSOSYS1");
        Result = pw.getPassTicket();
        System.out.println("Passticket is "+Result);
    }
    catch ( Exception e)
    {
      Class c = e.getClass();
      String s = c.getName();
      System.out.println("Exception >>>"+s+"  "+e.getMessage());
      e.printStackTrace();
    }
   }
}
